...
class Graph(Container):
    ...
    def forward(self, device, *args):
        '''
        Computes forward pass on input network. Device is a string value which 
        indicates the device on which to compute the device. If the device is 
        'cpu', then it will simply perform the forward pass without moving 
        any of the torch and crypten tensors to the GPU. Otherwise, it will 
        move the tensors to the specified device
        '''
        
        # print("[DEBUG-nn.module.py 657]: comp graph = {}".format(self._graph))
        # return
        ...
        for key, value_list in self._graph.items():
            if '' in value_list: 
                ...
                for i in range(len(value_list)):
                    vals.append(True) if value_list[i] == '' else vals.append(False)
                inputs_available[key] = vals
            else:
                inputs_available[key] = [False for _ in range(len(value_list))] 
        ...
        while node_to_compute is not None:
            ...
            # NEW CODE: (see 669-684 for preceding modification)
            # NEW CODE: check if the input type is a tensor, if it is send to cuda

            # print("[DEBUG-nn/module.py - 735] - new input method")
           
            # NEW CODE ---------------------------------------------------------------------------------------
            if not device == 'cpu' and device is not None:
                input = []
                for name in self._graph[node_to_compute]:
                    if name in values:
                        # print("\t[DEBUG-nn/module.py - 740]: input type = {}".format(type(values[name])))
                        #if isinstance(values[name], crypten.mpc.mpc.MPCTensor): 
                        #    print("[DEBUG-757]: cpu values = {}".format(values[name]))
                        #    print("[DEBUG-758]: {} values = {}".format(device, values[name].to(device)))
                        #    input.append(values[name].to(device))
                            # print("\t\t[DEBUG-nn/module.py - 743]: moved crypten input to cuda")
                        if isinstance(values[name], torch.Tensor):
                            # print("\t\t[DEBUG-nn/module.py - 745]: moved torch input to cuda")
                            input.append(values[name].to(device))
                            # print("[DEBUG-757]: cpu values = {}".format(values[name]))
                            # print("[DEBUG-758]: {} values = {}".format(device, values[name].to(device)))
                        else: input.append(values[name])
                    else: input.append(None)
            else:
                input = [values[name] if name in values else None for name in self._graph[node_to_compute]]
            # NEW CODE ---------------------------------------------------------------------------------------
            ...
            # print("[DEBUG-740 nn/module.py]: moving module to cuda")
            # module.cuda()

            # print("[DEBUG-line740 in crypten/nn/module.py]: module device = {}".format(module.device))
        ...
        # DEBUG
        if node_to_compute is None: 
            print("[DEBUG]: nodes to compute still\n{}".format(computed))
            print("[DEBUG]: inputs available at execution\n{}".format(inputs_available))
...
class Floor(Module):
    """
    Implementation for ONNX 13 Floor

    NOTE: NOT FINISHED. MISSING FLOOR IMPLEMENTATION IN THE BASE
    MPC TENSOR FUNCTIONALITY
    """
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # DUMMY CODE IMPLEMENTATION, when 
        # MPC tensor function .floor() is implemented
        # change this code to reflect that change
        return x
...
class Resize(Module):
    """
    Performs ONNX 13 specification Resizing operation.

    This implementation is only supported for 4-D tensors
    of shape (mini-batch, channels, height, width)

    Default parameters are set for what is supported by 
    ...
    def __init__(self, 
        ...
        super().__init__()
        self.coord_trans_mode = coord_trans_mode
    ...
    def forward(self, x):
        if self.mode != 'nearest': 
            raise ValueError("mode = {} -> only nearest upsampling is available".format(self.mode))
        elif self.coord_trans_mode != 'asymmetric':
            raise ValueError("only asymmetric upsampling is supported")
        elif self.nearest_mode != 'floor':
            raise ValueError("nearest pixels are obtained only by rounding down to the nearest integer")

        # check inputs
        if isinstance(x, list) or isinstance(x, tuple):
            if len(x) == 1: 
                raise ValueError("Must pass at least 2 arguments")
            elif len(x) == 2:
                raise ValueError("roi based resize not supported")
            elif len(x) == 3:
                raise ValueError("scales based resize not supported")
            elif len(x) == 4:
                x, _, _, sizes = x
        else:
            raise ValueError("Need to adjust code line 2843 in crypten/nn/module.py in python3.9 site-packages (crypten_env)")
        
        if len(x.shape) != 4: 
            raise ValueError("x.shape = {} -> Only 4-D tensors of shape (mini-batch, channels, height, width) are supported".format(x.shape))
        if (len(sizes) != 2) and (len(sizes) != 4): # if the window is size 4, need to extract the window from the last two dimensions
            raise ValueError("x.shape = {} sizes = {} -> Only windows of shape (height, width) are supported".format(x.shape, sizes))
        
        # Catch for if the input is of size 4 instead of a tuple with 2 values
        if len(sizes) == 4: 
            sizes = sizes[2:].tolist()

        batch_size, channels, orig_height, orig_width = x.shape
        target_height, target_width = sizes

        scale_height = target_height / orig_height
        scale_width = target_width / orig_width

        grid_y, grid_x = torch.meshgrid(torch.arange(target_height), torch.arange(target_width), indexing='ij')

        orig_y = (grid_y // scale_height).long()
        orig_x = (grid_x // scale_width).long()

        upsampled_tensor = x[:,:,orig_y,orig_x]
    ...
    def from_onnx(attributes=None):
        ...
        # check if the argument was passed as a byte string or not  
...
# TODO: finish implementation for Split module
# NOTE: 7/16/24 - Yoshi234 - check correctness of implementation
class Split(Module):
    """
    NOTE: it may be necessary to manually adjust the default
    value of axis, depending on the module. 
    
    Performs ONNX split operation according to ONNX 11 
    specification. Splits a tensor into a list of tensors, 
    along the specified 'axis'. Lengths of the parts can be specified
    the 'split' input. Unlike the ONNX specification, the 'split' 
    argument must be specified in order for the operation to work
    properly. Implementation for "equal parts" division of the tensor
    into separate parts along a specified dimension
    ...
    def forward(self, x):
        if isinstance(x, list):
            if len(x) == 1:
                raise ValueError("'split' argument must be provided")
            elif len(x) == 2:
                x, split = x
                axis = self.axis
            elif len(x) == 3:
                ...
                raise ValueError("list input x must have 2 or 3 values")
            split = split.int().tolist() # assumes that split is a torch tensor
        else:
            print("[DEBUG-crypten.nn.Resize()]: input type = {}".format(type(x)))
            raise ValueError("list input must have at least 2 values, 'split' arg is required.")

        # DEBUG
        # print("[DEBUG]: x.shape = {} \naxis = {}\nsizes = {}".format(x.shape, axis, split))
        
        tensors = []
        start = 0
        for size in split:
            tmp = x.narrow(int(axis), int(start), int(size))
            start += size
            tensors.append(tmp)

        # expected output should be a tuple, not a list
